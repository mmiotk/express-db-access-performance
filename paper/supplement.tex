...
\noindent This supplement carries measurement detail referenced from the main text. The archived published baseline is release v1.12.13 under the Zenodo concept DOI (\url{https://doi.org/10.5281/zenodo.21313858}); its primary-campaign tables are unchanged from release v1.12.12 (version DOI \url{https://doi.org/10.5281/zenodo.21498786}). This revision candidate adds the specification-derived oracle, campaign-state preflight, a replacement corrected-state primary campaign, a second same-host RQ2 validation campaign, fresh secondary measurements, the external-audit dataset, and blind review packets. The revised primary tables are generated from the corrected-state campaign rather than inherited from the archived baseline. It is not described as archived until a new immutable version DOI is minted before submission.
...
{\small
  ...
  \label{tab:reproducibility}\\
    ...
    Item & Detail \\
    \midrule
    Artifact \& version & Published baseline: v1.12.13. This unarchived revision candidate adds the seed-specification oracle, corrected-state 90-cell primary campaign, 42-cell same-host validation campaign, fresh secondary measurements, external-audit dataset, and blind review packets. A new immutable release is required before submission. \\
    Code provenance & Git commit and dirty status, package-lock SHA-256, plus aggregate and per-file SHA-256 for 42 measurement, admission, setup, state-control, schema, and adapter files. Exact 42-file source archives are preserved for both accepted campaigns and verify member-by-member against their embedded manifests. The accepted primary aggregate is \texttt{532d9c7ffe94\ldots d06cf228}; the independently ordered same-host validation aggregate is \texttt{d32a32b5b68a\ldots 134d3789a}. Full values are retained in the source manifests. \\
    Persistent identifier & Zenodo concept DOI \texttt{10.5281/zenodo.21313858}, currently resolving to published v1.12.13; the candidate commit and future version DOI must be recorded before submission. \\
