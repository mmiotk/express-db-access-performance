...
\noindent This supplement carries measurement detail referenced from the main text. The complete revision artifact is archived as release v1.13.14, which the concept DOI \url{https://doi.org/10.5281/zenodo.21313858} resolves to; the Zenodo record also carries an immutable per-version DOI for that release. This release adds the specification-derived oracle, campaign-state preflight, a replacement corrected-state primary campaign, a second same-host RQ2 validation campaign, fresh secondary measurements, the external-audit dataset, and blind review packets. The revised primary tables are generated from the corrected-state campaign rather than inherited from the earlier baseline.
...
{\small
  ...
  \label{tab:reproducibility}\\
    ...
    Item & Detail \\
    \midrule
    Artifact \& version & Complete revision artifact v1.13.14: seed-specification oracle, corrected-state 90-cell primary campaign, 42-cell same-host validation campaign, fresh secondary measurements, external-audit dataset, and blind review packets. \\
    Code provenance & Git tag \texttt{v1.13.14} identifies the exact release commit, also recorded in the GitHub and Zenodo metadata. The package-lock SHA-256 and aggregate/per-file SHA-256 cover 42 measurement, admission, setup, state-control, schema, and adapter files. Exact 42-file source archives are preserved for both accepted campaigns and verify member-by-member against their embedded manifests. The accepted primary aggregate is \texttt{532d9c7ffe94\ldots d06cf228}; the independently ordered same-host validation aggregate is \texttt{d32a32b5b68a\ldots 134d3789a}. Full values are retained in the source manifests. \\
    Persistent identifier & Concept DOI \texttt{10.5281/zenodo.21313858}, resolving to release v1.13.14; the Zenodo record also carries an immutable per-version DOI. \\
    ...
    Verification status & On 1 August 2026, an author-run isolated reconstruction from a clean export of the tagged commit verified 65/65 archived JSON hashes and regenerated all 58 manuscript tables byte-for-byte without starting either DBMS. The earlier 26 July run covered the 60 datasets listed at that date; five were added to the manifest afterwards. The earlier author-run archive-isolated v1.12.9 check verified 35/35 inputs and 45/50 then-committed tables; its five presentation-only differences are named in \texttt{notes/clean-room-reproduction.md}. Neither check is independent reproduction or a re-execution of the current measurements. \\
...
correction that disclosure leaves open. The correction is matched to what the rule
claims, which is a conjunction: the pair's ratio differs from~1 on both stacks, in opposite
...
the result is Holm-corrected across all 63 pair-pattern screens in a campaign. Holm retains 53 of 63
in the primary campaign and 50 of 63 in the validation campaign; applying the direction and margin
filters to the survivors leaves exactly the four pairs the screen promoted, and in the validation
campaign those four plus one that the recurrence requirement excludes. The sign-flip null here is
...
does not settle is worth stating plainly: it removes multiple testing as an explanation for the four
promoted pairs,
