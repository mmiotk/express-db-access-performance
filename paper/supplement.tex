...
\noindent This supplement carries measurement detail referenced from the main text. The complete revision artifact is archived as release v1.12.17 under the immutable version DOI \url{https://doi.org/10.5281/zenodo.21610058}; the concept DOI is \url{https://doi.org/10.5281/zenodo.21313858}. This release adds the specification-derived oracle, campaign-state preflight, a replacement corrected-state primary campaign, a second same-host RQ2 validation campaign, fresh secondary measurements, the external-audit dataset, and blind review packets. The revised primary tables are generated from the corrected-state campaign rather than inherited from the earlier baseline.
...
{\small
  ...
  \label{tab:reproducibility}\\
    ...
    Item & Detail \\
    \midrule
    Artifact \& version & Complete revision artifact v1.12.17: seed-specification oracle, corrected-state 90-cell primary campaign, 42-cell same-host validation campaign, fresh secondary measurements, external-audit dataset, and blind review packets. \\
    Code provenance & Git tag \texttt{v1.12.17} identifies the exact release commit, also recorded in the GitHub and Zenodo metadata. The package-lock SHA-256 and aggregate/per-file SHA-256 cover 42 measurement, admission, setup, state-control, schema, and adapter files. Exact 42-file source archives are preserved for both accepted campaigns and verify member-by-member against their embedded manifests. The accepted primary aggregate is \texttt{532d9c7ffe94\ldots d06cf228}; the independently ordered same-host validation aggregate is \texttt{d32a32b5b68a\ldots 134d3789a}. Full values are retained in the source manifests. \\
    Persistent identifier & Immutable Zenodo version DOI \texttt{10.5281/zenodo.21610058}; concept DOI \texttt{10.5281/zenodo.21313858}. \\
...
instantiation rather than a claim of full-matrix coverage. The sixth, independent-implementation-review level is not satisfied: packets exist under \texttt{notes/reviewer-packets/}, but no external human review is claimed. Stating the levels and their coverage
...
promotion rule is unchanged. No pair is promoted, while cross-stack rank agreement does not
improve and on aggregation falls. Holding out Prisma removes the driver substitution and the
...
\section{The comparability protocol at a glance}
\label{sup:protocol-figure}
Figure~\ref{fig:protocol} states the protocol's inputs, its mandatory and recommended stages, the
cell-admission rule, and how each stage's output should be interpreted. It is the diagrammatic form of
the protocol specified in Section~4 and encoded in \texttt{protocol-checklist.yaml}.
