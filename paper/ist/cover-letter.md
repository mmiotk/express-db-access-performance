...
Following the presentation review, the central protocol is now carried by a one-page **protocol flow figure** (Figure 1) in place of the earlier framed text box, a new **estimands table** consolidates in one place what each quantity does and does not identify, a new **analysis-roster table** separates the primary analyses from the robustness/sensitivity ones, the repeated defense of the same-SQL comparison is stated once with back-references, and three secondary result tables were moved to the online supplement — the main-text float count is unchanged at seven.

The manuscript is **14,530 words** under the IST counting rule (references and appendices counted, each main-text float 200 words), within the 15,000-word Research Paper limit, with a five-part structured abstract of ~292 words; the exact word-equivalent calculation the reviewer requested is documented in the accompanying word-count declaration. The full point-by-point reply, each quoting the revised text, is in the accompanying Response to Reviewers, and a Summary of Revisions accompanies it. The complete replication package remains public on GitHub and is permanently archived on Zenodo (DOI: 10.5281/zenodo.21491886); the numbered online supplement accompanies the manuscript.
