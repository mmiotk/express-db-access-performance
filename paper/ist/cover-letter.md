...
Following the presentation review, the central protocol is now carried by a one-page **protocol flow figure** (Figure 1) in place of the earlier framed text box, a new **estimands table** consolidates in one place what each quantity does and does not identify, a new **analysis-roster table** separates the primary analyses from the robustness/sensitivity ones, the repeated defense of the same-SQL comparison is stated once with back-references, and (following the final consolidated list) the two deep-fetch regimes are presented side by side in the main text (Table 7) while other secondary tables moved to the online supplement; a machine-readable protocol checklist now ships with the replication package.

The manuscript is **14,942 words** under the IST counting rule (references and appendices counted, each main-text float 200 words), within the 15,000-word Research Paper limit, with a five-part structured abstract of ~299 words; the exact word-equivalent calculation the reviewer requested is documented in the accompanying word-count declaration. The full point-by-point reply, each quoting the revised text, is in the accompanying Response to Reviewers, and a Summary of Revisions accompanies it. The complete replication package remains public on GitHub and is permanently archived on Zenodo (DOI: 10.5281/zenodo.21495887); the numbered online supplement accompanies the manuscript.
