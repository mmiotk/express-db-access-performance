...
\begin{itemize}
  ...
  \item \textbf{Recoverable provenance.} Assistance is recorded per commit, not asserted in aggregate: every AI-assisted commit carries a \texttt{Co-Authored-By} trailer, so a reviewer can re-derive the distribution with \texttt{git log --grep="Co-Authored-By: Claude"}. We prefer this to a prose claim because it is checkable against the public history.
