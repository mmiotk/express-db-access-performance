...
| Component | Count |
|---|---:|
| Seven body sections (`texcount` sum, including section headings and caption text encountered in those files) | 11,206 |
...
| Rendered reference list (65 entries; conservative `pdftotext` count from the first entry to the end) | 2,000 |
| **Conservative IST submission equivalent** | **14,980** |

The controlling total is therefore **14,980 words**, leaving **20 words of headroom** under the 15,000-word limit. The structured abstract contains 222 prose words (227 under the `texcount` sum convention), below 300 words.
...
**The remaining headroom is 20 words.** Any further addition requires an offsetting cut, and this declaration must be regenerated with it.
