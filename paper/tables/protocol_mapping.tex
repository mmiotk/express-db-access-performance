\begin{table}[htbp]
  ...
  \begin{tabular}{p{1.7cm} p{2.3cm} p{2.5cm} p{3.3cm} p{2.5cm} p{3.4cm}}
    ...
    At 50 conns deep-fetch p99 runs pg 20\,ms to MikroORM 116\,ms; at 50\% of each layer's own capacity the tails converge to $\sim$2--5\,ms, so the gap is capacity-and-queueing, not a sub-saturation latency difference \\
    \bottomrule
  \end{tabular}
  \end{adjustbox}
\end{table}
