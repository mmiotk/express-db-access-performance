...
- [x] Zenodo v1.13.10 deposit prepared; its per-version DOI is recorded in the Zenodo record.
      From v1.13.10 the papers cite the concept DOI, so an archive can identify itself without
...
- [ ] **Archive v1.13.10 on Zenodo, then verify with `scripts/release.sh --check-doi`.**
      As of 2026-07-30 the concept DOI still resolves to the previous release (26 July) rather than
      to v1.13.10, so the Data Availability sentence "archived as release v1.13.10, which the concept
      DOI resolves to" is **currently false**. The GitHub release for v1.13.10 exists but did not
      ...
      mechanism here. Create a new version of the existing record, upload the v1.13.10 archive,
