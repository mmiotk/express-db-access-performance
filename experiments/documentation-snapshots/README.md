...
This directory makes the policy-selected documented treatment auditable without relying
on the current documentation websites.

- manifest.json records the freeze date, exact source URL, selection priority,
  Internet Archive capture timestamp and URL, SHA-256, byte length, evidence
  terms, pinned version, selected API, alternatives, tie-break status, and decision
  basis for every layer.
...
- `node ../scripts/archive-documentation.mjs --verify` checks IDs, source URLs, byte lengths, SHA-256 hashes, evidence terms, capture dates, and exact decision records without network access.
- `node ../scripts/archive-documentation.mjs` regenerates the archive. A regenerated
...
predeclared documented-base-layer tie-break. If that still fails, the assignment is marked
