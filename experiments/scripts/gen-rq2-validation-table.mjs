...
const output = {
  scope: "Descriptive comparison of two corrected-state campaigns on one host. Campaign dates are not paired. Headline reversals must recur, exceed the plus or minus 5 percent margin on both stacks, and have paired-bootstrap intervals excluding equality in opposite directions in both campaigns. No cross-host transfer is inferred.",
...
\\begin{table}[htbp]
  \\centering
  \\small
  \\caption{Same-host, between-campaign sensitivity for the reviewer-prioritized RQ2 patterns. Both corrected-state campaigns use 25 repetitions per cell; the validation campaign uses a new block order and environment fingerprint. Leaders and Spearman rank agreement compare campaign medians for the seven portable layers. Exact ranks counts unchanged rank positions; drift is the largest absolute layer-level median-throughput change. A reversal is promoted in RQ2 only when it recurs, both stack-specific pair gaps exceed 5\\%, and paired-bootstrap intervals exclude equality in opposite directions in both campaigns. The margin predates these campaigns; the composite rule was specified after both completed and is applied uniformly to all pairs, so promotion is a post-hoc screen rather than a prespecified confirmatory test. Pairs meeting all criteria: ${robustReversalText}. Campaign dates are not paired, and this is not cross-host replication.}
