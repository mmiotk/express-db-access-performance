...
const exclNote = excluded.length
  ? ` \\texttt{${excluded.join('}, \\texttt{')}} ${one ? 'is' : 'are'} omitted: ${one ? 'its' : 'their'} alternative strategy was not a byte-identical drop-in in the pinned versions (TypeORM's query strategy errors on the ordered deep-fetch \\texttt{findOne}; Objection's \\texttt{withGraphJoined} changes row handling), which is itself a portability caveat.`
  : '';
