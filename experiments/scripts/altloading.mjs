...
const exclNote = excluded.length
  ? ` \\texttt{${excluded.join('}, \\texttt{')}} are omitted: their alternative strategy was not a byte-identical drop-in in the pinned versions (TypeORM's query strategy errors on the ordered deep-fetch \\texttt{findOne}; Objection's \\texttt{withGraphJoined} changes row handling), which is itself a portability caveat.`
  : '';
...
\\begin{table}[htbp]
  ...
  \\caption{Alternative eager-loading sensitivity on the deep fetch:
    ...
    configuration-portability findings rather than timed treatments.${exclNote}}
