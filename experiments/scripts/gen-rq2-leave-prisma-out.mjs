...
// The four promoted reversals are the headline RQ2 result, and until now they were named
// but never sized: no ratio and no interval appeared in any table. Reporting an effect
// without dispersion is exactly what this paper's own standard forbids.
const fmt = (x) => x.toFixed(3);
const ci = (a) => `[${a[0].toFixed(3)}, ${a[1].toFixed(3)}]`;
const promoted = [];
for (const key of Object.keys(output.full)) {
  const e = output.full[key];
  for (const p of e.promotedDetail || []) {
    promoted.push(`    ${e.label} & \\texttt{${p.pair.replace("|", "} vs \\texttt{")}} `
      + `& ${fmt(p.pgRatio)} & ${ci(p.pgRatioCI)} & ${fmt(p.mysqlRatio)} & ${ci(p.mysqlRatioCI)} \\\\`);
  }
}
const promotedTex = `% auto-generated by scripts/gen-rq2-leave-prisma-out.mjs
\\begin{table}[htbp]
  \\centering
  \\caption{Effect sizes for the ${promoted.length} promoted cross-stack reversals. Each row gives the
...
`;
for (const dir of [path.join(resultsDir, "tables"), paperDir]) {
  fs.writeFileSync(path.join(dir, "rq2_promoted_effects.tex"), promotedTex);
}
console.log(`promoted reversals: all seven = ${output.promotedCountFull}, Prisma held out = ${output.promotedCountReduced}`);
console.log(`wrote rq2_promoted_effects.tex (${promoted.length} rows)`);
