  ...
  for (const [endpoint] of PATTERNS) {
    for (let i = 0; i < LAYERS.length; i++) {
      for (let j = i + 1; j < LAYERS.length; j++) {
        ...
        const pPG = signFlipP(logRatios("postgres"), mulberry32(0x5eed));
        const pMY = signFlipP(logRatios("mysql"), mulberry32(0x5eed));
...
\\begin{table}[htbp]
  ...
  \\caption{Familywise correction matched to the promotion rule. The rule claims a
    ...
    ($B={${B}}$, seed \\texttt{${out.seed}}), the two combined by taking the larger $p$, and the
