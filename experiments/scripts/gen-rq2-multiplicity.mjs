...
\\begin{table}[htbp]
  ...
  \\caption{Familywise correction of the per-stack nonzero-effect evidence behind the promoted
    ...
    ($B={${B}}$, per-test seeds derived from campaign, pattern, pair and engine), the two combined
    by intersection-union (the larger $p$), and the result Holm-corrected across all
    ${out.campaigns.primary.tests} pair-pattern screens in a campaign. What is \\emph{not}
    ...
    ${out.campaigns.primary.selectedAfterFilters.length} pairs the screen promoted in the primary
