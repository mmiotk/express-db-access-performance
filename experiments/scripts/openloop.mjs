...
// the deep/nested fetch on PostgreSQL at a FIXED aggregate request rate
...
\\begin{table}[htbp]
  \\centering
  \\caption{Open-loop tail latency: p99 (ms) on the deep/nested fetch (PostgreSQL) under a constant offered request rate (${CONNECTIONS} connections, autocannon \\texttt{overallRate}). $^\\dagger$marks cells where the layer could not sustain the offered rate (achieved${'<'}95\\%), i.e.\\ it saturated below that load.}
  \\label{tab:openloop}
