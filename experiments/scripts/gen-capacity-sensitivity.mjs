...
const utilization = [
  ...(await readJson(process.env.UTIL_PG_FILE || 'utilization-corrected.postgres.json')),
  ...(await readJson(process.env.UTIL_MY_FILE || 'utilization-corrected.mysql.json')),
];
const scaling = await readJson(process.env.SCALING_FILE || 'scaling-corrected.json');
const raw = await readJson(process.env.RAW_FILE || 'current-primary.json');

const capacities = new Map();
for (const row of utilization) {
  const key = `${row.engine}/${row.adapter}`;
  if (!Array.isArray(row.p99_samples) || row.p99_samples.length < 2) {
    throw new Error('utilization run lacks repeated p99 samples for ' + key + ' at fraction=' + row.fraction);
  }
  if (Number(row.errors_total ?? 0) !== 0) {
    throw new Error('utilization run contains non-2xx responses for ' + key + ' at fraction=' + row.fraction);
  }
...
const sweepPoints = new Map();
for (const row of scaling) {
  const key = `${row.engine}/${row.adapter}`;
  if (Number(row.errors ?? 0) !== 0 || Number(row.timeouts ?? 0) !== 0 || Number(row.non2xx ?? 0) !== 0) {
    throw new Error('capacity sweep contains failed requests for ' + key + ' at c=' + row.connections);
  }
  ...
  if (!Array.isArray(row.rps_samples) || row.rps_samples.length < 2) {
    throw new Error(`capacity sweep lacks repeated raw samples for ${key} at c=${row.connections}`);
  }
  const points = sweepPoints.get(key) ?? [];
  points.push(row);
  sweepPoints.set(key, points);
}
const sweepMaxSamples = new Map();
for (const [key, points] of sweepPoints) {
  const repetitions = Math.min(...points.map((point) => point.rps_samples.length));
  sweepMaxSamples.set(key, Array.from({ length: repetitions }, (_, replicate) =>
    Math.max(...points.map((point) => point.rps_samples[replicate]))));
}
...
const expectedConnections = [...new Set(scaling.filter((row) => order.includes(row.engine + '/' + row.adapter)).map((row) => row.connections))].sort((a, b) => a - b);
if (expectedConnections.length < 3) throw new Error('capacity sweep ladder is incomplete');
for (const key of order) {
  const points = sweepPoints.get(key) ?? [];
  const connections = points.map((row) => row.connections).sort((a, b) => a - b);
  if (new Set(connections).size !== connections.length || JSON.stringify(connections) !== JSON.stringify(expectedConnections)) {
    throw new Error('capacity sweep point mismatch for ' + key);
  }
}
...
const rows = order.map((key) => {
  ...
  return {
    ...
    capacity,
    sweepMax,
    sweepMaxSamples: sweepMaxSamples.get(key),
    ratio,
...
const sweepRepeat50 = rows.flatMap((row) => row.sweepMaxSamples.map((sample) => 0.5 * row.capacity / sample));
const sweepRepeat70 = sweepRepeat50.map((fraction) => 1.4 * fraction);
...
\\begin{table}[htbp]
  ...
  \\caption{Sensitivity of the matched-utilization denominator on the deep fetch.
    ...
    ${pct(Math.min(...sweepRepeat50))}--${pct(Math.max(...sweepRepeat50))}\\% and
    ${pct(Math.min(...sweepRepeat70))}--${pct(Math.max(...sweepRepeat70))}\\%.
...
await writeFile(join(results, "capacity-sensitivity.json"), JSON.stringify({
  capacity_proxy: "median throughput at 50 connections",
  sweep_denominator: "maximum median throughput on the full concurrency ladder",
  rows,
  global_bands: { bootstrap50, bootstrap70, sweepRepeat50, sweepRepeat70 },
}, null, 2) + "\n");
