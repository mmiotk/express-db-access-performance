...
for (const { engine, adapter } of CELLS) {
  ...
  try {
    await health(base);
    await ac({ url: `${base}/posts/1/thread`, duration: 3 }); await waitIdle(base);   // re-warm cold buffers
    const deep = [];
    for (let i = 0; i < REPEATS; i++) { deep.push(await ac({ url: `${base}/posts/1/thread` })); await waitIdle(base); }
