...
\\begin{table}[htbp]
  ...
  \\caption{Equivalence assessment for the closest adjacent policy-selected pair on the deep
    ...
    The pair is chosen from the data as the smallest adjacent ratio of cell medians among the nine
    policy-selected layers, so the selection is post hoc; the tuned baselines are excluded as
