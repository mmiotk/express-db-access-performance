...
// (fixed post id, warmup pass, then 25 timed repeats), so the post-hoc outcome-selected ORM
...
// results/deepfetch-regimes.json (one file for the whole legacy post-hoc sensitivity), so
...
for (const { engine, adapter } of CELLS) {
  ...
  try {
    await health(base);
    await run('/posts/1/thread', base, WARMUP); await waitIdle(base);
    const doc = [];
...
// append the native reference rows to the legacy sensitivity file (one raw-data file for the
