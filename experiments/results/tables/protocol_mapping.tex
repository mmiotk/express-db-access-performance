\begin{table}[htbp]
  ...
  \begin{tabular}{p{1.7cm} p{2.3cm} p{2.5cm} p{3.3cm} p{2.5cm} p{3.4cm}}
    ...
    Mis-read the whole documentation-selected spread as library-only ``overhead'' &
    The $7.15\times$/$4.96\times$ spread narrows to $1.68\times$ (PG) / $2.01\times$ (MySQL) among raw paths on common SQL under this compound raw-SQL standardization; which component is responsible is not identified \\
    \addlinespace
    ...
    At 50 conns deep-fetch p99 runs pg 20\,ms to MikroORM 116\,ms; at nominal 50\% of the $\widehat C_{50}$ capacity proxy the tails converge to $\sim$2--5\,ms, and the full-sweep-denominator sensitivity stays below saturation (S45) \\
    \bottomrule
  \end{tabular}
  \end{adjustbox}
\end{table}
